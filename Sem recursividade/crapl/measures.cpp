...
/*int last_funct(){
	...
	//size = 0;
		...
		for(int x = 0; x < nr_funcs; x++){
			...
			recurs.insert(std::pair<int,int>(x,0));
		}
	}
...
/*void insert(char const * fn, float pack, float pp0, float pp1, float dram, float cl){
	...
	void decrementRecur(int fn){
		std::map<int, int>::iterator i = recurs.find(fn);
		i->second -= 1;
		recurs[fn] = i->second;
		//printf("Decr %d - %d\n", fn, i->second);
	}

	bool checkRecursive(int fn){
		std::map<int, int>::iterator i = recurs.find(fn);
		//printf("Check %d - %d\n", fn, i->second);
		if(i->second > 1)
			return true;
	...
	//printf("pack: %.9f - pp0: %.9f - pp1: %.9f - dram: %.9f - cl: %.9f \n", pack, pp0, pp1, dram, cl);
			...
			*/
			...
			results[fn][4] += cl;
			/*
		results[fn][0] += (pack * 0.0003);
		results[fn][1] += (pp0 * 0.0002);
		results[fn][2] += (pp1  * 0.000015);
		results[fn][3] += (dram * 0.0004);
		results[fn][4] += cl;*/
	}
	//size++;
